...
import com.tromke.mydrive.Utils.MyFirebaseInstanceIDService;
...
public class ActLogin extends AppCompatActivity implements IntLoginView {
    ...
    @BindView(R.id.edt_email_id1)
    EditText edt_email_id;

    @BindView(R.id.edt_password_id1)
    ...
    public void registration() {
        //SignUp to server
        presenter.attemptRegister(edt_email_id.getText().toString(),
    ...
    public void validationPasswordSuccess() {
        if (ConnectionManager.getInstance(getApplicationContext()).isDeviceConnectedToInternet()) {
            ...
            //authenticate user
            FirebaseAuth.getInstance().signInWithEmailAndPassword(edt_email_id.getText().toString(), edt_password_id.getText().toString())
    ...
    public void getHyperTrackIdForDriver() {
        ...
        ref = new Firebase(Config.FIREBASE_URL).child("Driver");
        ...
        queryRef.addChildEventListener(new ChildEventListener() {
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String previousChild) {
                if (dataSnapshot.exists()) {
                    ...
                    if (MyFirebaseInstanceIDService.TOKEN_CHANGED == 1) {
                        String key = dataSnapshot.getKey();
                        ref.child(key).child("notification_token").setValue(FirebaseInstanceId.getInstance().getToken());
//                        Log.d(TAG, "Your Token Changed");
                    }
                    if (driverId.equals(userId.toString()) && !hypertrack_id.equals("null")) {
                        ...
                        intent.putExtra("checkstate", hypertrack_id);
